...
4. **Read and reconcile**
   ...
   - Revise only files that already exist (`existingOutputPaths`). Do NOT create artifacts that don't exist yet, and do NOT invent new files under a glob artifact. Note them and point the user to the OpenSpec artifact instructions needed to create them.
...
6. **Point to the next step (guidance only - NEVER act on it)**
   - Artifacts still missing -> suggest using OpenSpec artifact instructions to create them.
...
- Anything deferred to later artifact creation (not-yet-created artifacts or files)
...
- Do not advance the build frontier: no new artifacts and no new files under glob artifacts. Use the CLI-reported next artifact and its instructions for creation.
- Confirm every edit with the user before writing.
- If the request changes the change's *intent* rather than refining it, recommend starting a fresh change (the "Update vs. Start Fresh" heuristic).
- `openspec status --change "<name>" --json` shows the next artifact and `openspec instructions <artifact-id> --change "<name>" --json` explains how to create it.
