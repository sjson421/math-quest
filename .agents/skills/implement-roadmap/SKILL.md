...
Create an `update_plan` plan with one `in_progress` entry named Apply. This skill is intended
to run in a session using `gpt-5.6-luna` with max reasoning. Implement in the current
session; do not spawn a replacement implementation agent solely to change models.
