...
Use `simplify` on this run's changed paths. Follow the agent contract in `SKILL.md`: one
reviewer by default, with two or three permitted only for non-overlapping path domains. Each
domain review checks reuse, quality, and efficiency rather than assigning perspectives over
the same files.
