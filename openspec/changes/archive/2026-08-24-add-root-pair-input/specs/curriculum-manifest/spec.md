...
**Reason**: Its coordinate-plane scenario freezes the historical state in which all 28 Stage F skills were planned and the playable total was 145. That is no longer true after Units 16, 17, and 18a shipped, so carrying the requirement forward under `MODIFIED` would archive a false scenario.

**Migration**: Replace it with `Stage capability requirements remain complete`, which retains the current capability contract and scenarios, removes the obsolete historical count, and adds available root-pair input for Stage F at the current playable total of 165.
