...
A copy of local progress kept on a small serverless endpoint, because iOS can
evict web app storage and losing months of streak is the worst failure this app has.

The device stays the working store: sync is additive, and the app is fully usable before,
during, and after a sync failure. Writes are last-write-wins behind a monotonic version, so a
stale device is refused rather than allowed to overwrite newer progress.
